...
public:
...
#ifdef SDL
	virtual void register_event_by_clock(DEVICE* device, int event_id, uint32 clock, bool loop, int* register_id)
#else
	...
	virtual void abort_main_cpu()
	{
		...
		event_manager->abort_main_cpu();
	}
	virtual void abort_sub_cpu()
	{
		if(event_manager == NULL) {
			event_manager = vm->first_device->next_device;
		}
